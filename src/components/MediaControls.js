...
const MediaControls = ({toggleChat, video,audio,toggleCam,toggleMic}) => {
    ...
    return (
     ...
     <Segment  vertical textAlign='center' inverted color="blue" >
        <Button size="huge" color={audio.enabled?"red":null}  circular icon={audio.enabled?'unmute':"mute"} onClick={toggleMic} />
        <Button size="huge" color={video.enabled?"red":null} circular icon='video camera' onClick={toggleCam} />
